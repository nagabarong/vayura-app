...
export default function LoginPage() {
    ...
    const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await apiFetch<{ token: string }>("/api/login", {
                method: "POST",
    ...
    return (
        <div
            className={`min-h-screen flex items-center justify-center p-4 transition-colors duration-500 ${darkMode ? "bg-gray-950" : "bg-gray-50"
                }`}
        >
            <Card
                className={`relative w-full max-w-6xl grid md:grid-cols-2 overflow-hidden rounded-[2rem] shadow-2xl border-0 transition-colors duration-500 ${darkMode ? "bg-gray-900 text-gray-100" : "bg-white text-gray-900"
                    }`}
            >
                ...
                <div className="flex flex-col justify-center px-10 lg:px-16 py-16">
                    <CardHeader className="p-0 mb-6">
                        ...
                        >
                            ...
                            <Link
                                href="/auth/register"
                                className={`underline ${darkMode ? "text-blue-400" : "text-primary"
                                    }`}
                            >
                                Sign up
                            </Link>
                ...
                <div className="relative bg-black flex flex-col items-center justify-end overflow-hidden rounded-[2rem] w-[550px] h-[600px]">
                    {images.map((img, i) => (
                        <Image
                            ...
                            fill
                            priority={i === 0}
                            className={`object-cover transition-opacity duration-700 absolute inset-0 ${i === index ? "opacity-100" : "opacity-0"
                                }`}
                        />
                    ))}
                    ...
                    <div className="relative z-10 flex flex-col items-center justify-end pb-10 text-center">
                        ...
                        <div className="flex justify-center gap-2">
                            {images.map((_, i) => (
                                <div
                                    key={i}
                                    className={`h-[3px] w-6 rounded-full transition-all duration-300 ${i === index ? "bg-white" : "bg-white/40"
                                        }`}
                                />
                            ))}
                        </div>
